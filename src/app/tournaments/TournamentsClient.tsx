...
import { Trophy, Users, Calendar, Clock, CheckCircle, XCircle, ExternalLink } from 'lucide-react'
...
export default function TournamentsClient({
  ...
  const [tournamentsWithCounts, setTournamentsWithCounts] =
    useState<Tournament[]>(initialTournaments)
  const [isLoading, setIsLoading] = useState(initialTournaments.length === 0)
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        ...
        if (!cancelled) setTournamentsWithCounts(fetched)
      } catch {
        // Keep SSR data
      } finally {
  ...
  if (isLoading && tournamentsWithCounts.length === 0) {
    return (
        ...
        <main className="flex-1 flex items-center justify-center container mx-auto px-4 py-8">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-slate-400 text-sm">
              {isEnglish ? 'Loading tournaments...' : 'Laster turneringer...'}
            </p>
          </div>
        </main>
      </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-slate-300 text-lg">
              {isEnglish
                ? 'EA FC Pro Clubs tournaments — registration, live matches and results'
                : 'EA FC Pro Clubs-turneringer — påmelding, live kamper og resultater'}
            </p>
            <p className="sr-only" lang={isEnglish ? 'nb' : 'en'}>
              {isEnglish
                ? 'PRO11 arrangerer Pro Clubs-turneringer i Norge med premiepotter og live resultater.'
                : 'PRO11 organizes Pro Clubs tournaments in Norway with prize pools and live results.'}
            </p>
          </div>
              ...
              <div className="pro11-card p-8 text-center">
                <p className="text-slate-300">
                  {isEnglish
                    ...
                    : 'Ingen turneringer tilgjengelig for øyeblikket.'}
                </p>
              </div>
            ) : (
              tournamentsWithCounts.map(tournament => (
                <article key={tournament.id} className="pro11-card p-6">
                    ...
                    <div className="md:col-span-2">
                      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span
                            className={`inline-flex items-center space-x-2 px-3 py-1 rounded-full text-sm font-semibold ${getStatusColor(tournament.status)}`}
                          >
                      ...
                      <p className="text-slate-300 mb-4">
                        {stripMetadataTags(
                          isEnglish && tournament.description_en
                    ...
                    <div className="flex flex-col space-y-3">
                      ...
                      {tournament.isDemo && (
                        <p className="text-purple-200 text-sm text-center px-2">
                          {isEnglish
                            ? 'Demo — registration closed'
                            : 'Demo — påmelding stengt'}
                        </p>
                      )}
                      ...
                      >
                        ...
                        <span>
                          {isEnglish ? 'Matches and results' : 'Kamper og resultater'}
                        </span>
                      </Link>
                    </div>
                  </div>
          ...
          <div className="pro11-card p-6 mt-8">
            <h2 className="text-xl font-semibold mb-4">
              {isEnglish ? 'Upcoming tournaments' : 'Kommende turneringer'}
            </h2>
            <p className="text-slate-300 mb-4">
              {isEnglish
